...
class DataModelFactory {
    // Dependencies
    private var _delegate as NitroxBLEDelegate;
    ...
    public function initialize(delegate as NitroxBLEDelegate, profileManager as ProfileManager) 
    {
        _delegate = delegate;
    ...
    public function getEnvironmentModel(device as Device) as NitroxProfileModel {
        var envModel = _envModel;
        if (envModel != null) {
            if (envModel.stillAlive()) {
                return (envModel.get() as NitroxProfileModel);
            }
        }

        var dataModel = new $.NitroxProfileModel(_delegate, _profileManager, device);
